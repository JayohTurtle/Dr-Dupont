...
["patient", "rdv", "actualite", "soin"].forEach(updateVisibleInput)
...
document.addEventListener("DOMContentLoaded", function () {
    function updateSoinFields(inputId, dataListId, hiddenId) {
        ...
        input.addEventListener("input", function () {
            let selectedOption = Array.from(dataList.options).find(option => option.value === input.value)
            if (selectedOption) {
                let soinId = selectedOption.getAttribute("data-id");
                hiddenField.value = soinId; // Met à jour l'input caché
                console.log("Soin sélectionné :", input.value, " - ID récupéré :", soinId);
            } else {
                hiddenField.value = ""; // Réinitialise si aucune correspondance
                console.log("Aucun soin correspondant trouvé !");
            }
        })
    }
